...
- [ ] `AkraBazzi/` - Akra-Bazzi theorem for analyzing divide-and-conquer recurrences: proves `T(n) ∈ Θ(n^p (1 + ∑_{u=0}^{n-1} g(n) / u^{p+1}))` for recurrences `T(n) = ∑ a_i T(r_i(n)) + g(n)` where `r_i(n) ≈ b_i n`; includes theory of polynomially-growing functions and sum transforms (pending)
