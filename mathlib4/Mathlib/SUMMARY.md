...
- [x] `Control/` - Functional programming abstractions: comprehensive functors/applicatives/monads framework with monad transformers (StateT/ReaderT/ExceptT/ContT/WriterT), bifunctors and bitraversable structures, traversable typeclass with lawful instances, multivariate functors (MvFunctor), equivalence functors, folding operations, random generation, fixed-point operators, universe lifting, and monadic normalization infrastructure (complete)
- [~] `Data/` - Data structures and types (lists, sets, finite types, bit vectors, numeric types) (preliminary)
