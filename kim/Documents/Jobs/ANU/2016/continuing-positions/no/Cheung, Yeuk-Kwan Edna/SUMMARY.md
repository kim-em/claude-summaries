---
source_path: /Users/kim/Documents/Jobs/ANU/2016/continuing-positions/no/Cheung, Yeuk-Kwan Edna
generated: 2026-01-26T05:30:00Z
git_sha: fafb26e72fe7b535873cd5b8a8f359b75df7fe29
git_branch: master
status: preliminary
files_count: 4
subdirs_count: 1
---

# Cheung, Yeuk-Kwan Edna

## Overview

Application materials from Dr. Yeuk-Kwan Edna Cheung for the 2016 ANU Mathematical Sciences Institute Associate Professor position (female-only continuing position). Dr. Cheung was a Professor at Nanjing University with a Ph.D. from Princeton (2000) specializing in string theory, cosmology, and N=4 Super Yang-Mills amplitudes. She required visa sponsorship (no right to work in Australia) and could start June 1, 2017. Her application included prestigious references from Ori Ganor (Berkeley), Robert Brandenberger (McGill), Edward Witten (IAS Princeton), and Rob Myers (Perimeter Institute).

## Key Files

| File | Purpose |
|------|---------|
| application.pdf | Complete 63-page application package including CV, cover letter, research statement, teaching philosophy, and publications list. Shows strong research record with 20+ publications and 600+ citations in top-tier journals. |
| ref Ganor.pdf | Reference letter from Ori Ganor (UC Berkeley) |
| ref Myers.pdf | Reference letter from Rob Myers (Perimeter Institute) |
| ref Vergados.pdf | Reference letter from Vergados |

## Subdirectories

- [x] `papers/` - Research papers (5 files)

## Search Tags

anu 2016 continuing-positions faculty-hiring female-only-position associate-professor edna-cheung yeuk-kwan-cheung nanjing-university princeton string-theory cosmology super-yang-mills visa-sponsorship reference-letters ganor brandenberger witten myers application-materials rejected-candidates
