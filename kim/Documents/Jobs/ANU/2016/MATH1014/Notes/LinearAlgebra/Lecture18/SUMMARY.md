---
source_path: /Users/kim/Documents/Jobs/ANU/2016/MATH1014/Notes/LinearAlgebra/Lecture18
generated: 2026-01-25T09:45:00Z
git_sha: fafb26e72fe7b535873cd5b8a8f359b75df7fe29
git_branch: master
status: preliminary
files_count: 8
subdirs_count: 1
---

# Lecture18

## Overview

LaTeX source files and generated PDFs for Lecture 18 on discrete dynamical systems in linear algebra. The lecture introduces discrete linear dynamical systems modeled by matrix equations (x_{k+1} = Ax_k), exploring their long-term behavior through eigenvalue analysis. Includes extensive worked examples: predator-prey systems (owl and woodrat populations under varying predation rates), graphical trajectory analysis showing attractors, repellors, saddle points, and spiral behavior, and ecological applications including buffalo herd survival and spotted owl population modeling. The lecture demonstrates how eigenvalue magnitudes and eigenvector decomposition determine whether populations grow, decay, reach equilibrium, or spiral.

## Key Files

| File | Purpose |
|------|---------|
| `MATH1014-LinearAlgebra-Lecture18.tex` | Main LaTeX source file covering discrete dynamical systems, predator-prey models, trajectory analysis, and ecological applications |
| `MATH1014-LinearAlgebra-Lecture18.slides.tex` | Generated slides version of the lecture content |
| `MATH1014-LinearAlgebra-Lecture18.handout.tex` | Generated handout version for printing |
| `MATH1014-LinearAlgebra-Lecture18.slides.pdf` | Compiled Beamer presentation slides (520 KB) |
| `MATH1014-LinearAlgebra-Lecture18.handout.pdf` | Compiled handout PDF in 3-up format (498 KB) |
| `MATH1014-LinearAlgebra-Lecture18.scribble.pdf` | Annotated version with handwritten notes/scribbles (3.1 MB) |

## Subdirectories

- [x] `diagrams/` - Supporting diagrams and images for visualizing dynamical systems

## Search Tags

math1014 linear-algebra discrete-dynamical-systems eigenvalues eigenvectors predator-prey trajectories attractors repellors saddle-points spiral-behavior population-dynamics spotted-owls buffalo-herd ecological-modeling beamer latex lecture18 anu 2016
