---
source_path: /Users/kim/Documents/Jobs/ANU/2015/1014
generated: 2025-12-04T00:00:00Z
git_sha: 48a99ef1be36fc0c3cca2af4518f1aebb71ec697
git_branch: master
status: preliminary
files_count: 9
subdirs_count: 7
---

# 1014

## Overview

Complete teaching materials and administrative records for MATH1014 (Mathematics and Applications 1) semester 2 2015 at ANU. Contains lecture notes, worksheets, exams and grading materials for a first-year mathematics course covering calculus and linear algebra. Includes detailed attendance tracking correlating lecture/tutorial attendance with final grades, comprehensive grading spreadsheets for ~280 students, supplementary exam results for students eligible for deferred assessments, student ID list from September 2015, course jokes collection for lectures, timetabling documents, makefile for building course materials, and symlink to web teaching directory. The folder structure separates content into Notes (lectures), Worksheets (problem sets), Exams (assessments), and EAP/OldMaterial/PastExams for supplementary resources.

## Key Files

| File | Purpose |
|------|---------|
| `20150921-attendance.txt` | List of 89 student IDs (u-numbers) from September 21, 2015 |
| `attendance-correlations.xlsx` | Comprehensive Excel analysis correlating lecture/tutorial attendance with final grades and exam performance (~156KB) |
| `final-grade-sheet.csv` | Official final grade sheet with student IDs, marks, letter grades, and supplementary exam eligibility (280+ students, includes marks, grade categories like HD/D/CR/P/N, PX codes for supplementary exams in calculus/linear algebra/both) |
| `final-grades.xlsx` | Excel version of final grades with additional formatting and calculations (~22KB) |
| `grades.xlsx` | Detailed grading workbook tracking individual assessment components throughout semester (~63KB) |
| `jokes` | Collection of 14 mathematics jokes and puns for use in lectures (includes classics like the vector/scalar cross product joke, Banach-Tarski anagram, and differential operator $e^x$ joke) |
| `makefile` | Build script that compiles Notes and Worksheets subdirectories |
| `supp-results-S2-2015.csv` | Supplementary exam results for ~30 students showing original marks (45-48), calculus/algebra component scores, final scores (typically 50), and final grades (P/NCN/DA) |
| `todo` | Single reminder note: "Lecture 4 is a bit short?" |
| `web` | Symbolic link to `/Users/scott/Sites/tqft.net/web/teaching/2015/1014` (web directory for online course materials) |

## Subdirectories

- [x] `EAP/` - Confidential student accommodation documents: Education Access Plans for disability accommodations and Elite Athlete plans (complete)
- [x] `Exams/` - Examination papers and related assessment materials (complete)
- [x] `Notes/` - Complete lecture materials comprising 25 lectures for MATH1014 linear algebra course, from vectors in ℝ³ through abstract vector spaces to eigenvalues, diagonalization, and applications (complete)
- [x] `OldMaterial/` - Comprehensive archive of MATH1014 materials from Semester 1 2013-2015: Joan Licata's 23-lecture linear algebra course with Beamer presentations and 28 Maple worksheets, complete 2014-2015 exam collection with solutions, 10 weekly WebAssign quizzes, and tutorial worksheets covering full syllabus - rich pedagogical resource combining theory, computation, and applications (complete)
- [x] `PastExams/` - Collection of past examination papers: 2014 final exam solutions for linear algebra covering eigenvalues, diagonalization, dynamical systems, and orthogonal projections (complete)
- [~] `Worksheets/` - Weekly tutorial worksheets for MATH1014 linear algebra: 11 problem sets (weeks 2-4, 6, 8-14) plus matrix workshop on eigenvalues and diagonalization, with LaTeX source and PDFs (preliminary)
- [ ] `timetabling/` - Timetabling documents including staff availability (62KB Word doc), students removed from tutorials list, course profile form (38KB) (pending)

## Search Tags

math1014 mathematics-and-applications-1 anu 2015 semester-2 first-year calculus linear-algebra lecture-notes worksheets exams grading attendance supplementary-exams student-records teaching-materials course-administration timetabling mathematics-jokes
