---
source_path: /Users/kim/Documents/Jobs/ANU/2015/1014/Notes/Lecture21
generated: 2025-12-04T06:30:00Z
git_sha: 48a99ef1be36fc0c3cca2af4518f1aebb71ec697
git_branch: master
status: preliminary
files_count: 3
subdirs_count: 1
---

# Lecture21

## Overview

Lecture 21 materials for MATH1014 focusing on orthogonal projections onto subspaces. The lecture generalizes the 2D/3D projection formula to higher-dimensional subspaces, covering the Orthogonal Decomposition Theorem (decomposing vectors as y = ŷ + z where ŷ ∈ W and z ∈ W⊥), the Best Approximation Theorem (showing ŷ is the closest vector in W to y), and the matrix formula proj_W(y) = UU^T y for orthonormal bases. Content includes multiple worked examples in ℝ³ and ℝ⁴, distance calculations from vectors to subspaces, and applications to finding orthogonal bases.

## Key Files

| File | Purpose |
|------|---------|
| `MATH1014-LinearAlgebra-Lecture21.tex` | LaTeX source for Lecture 21 on orthogonal projections (17KB, covers Orthogonal Decomposition Theorem with formula ŷ = Σ(y·u_i/u_i·u_i)u_i, Best Approximation Theorem proving ŷ is closest vector in W to y, projection matrix formula UU^T, worked examples in ℝ⁴ finding decompositions and distances) |
| `MATH1014-LinearAlgebra-Lecture21.pdf` | Compiled slides PDF (279KB, beamer presentation format) |
| `MATH1014-LinearAlgebra-Lecture21.scribble.pdf` | Annotated/scribbled version of slides (2.6MB, likely with handwritten annotations from live lecture) |

## Subdirectories

- [x] `diagrams/` - Supporting diagrams for geometric visualization of projections (4 PDF files: ex1.pdf, ex2.pdf, ex3.pdf for worked examples, la2201.pdf for Best Approximation Theorem illustration) (complete)

## Search Tags

math1014 linear-algebra lecture-21 orthogonal-projection subspaces orthogonal-decomposition-theorem best-approximation-theorem projection-matrix orthonormal-basis distance-to-subspace worked-examples beamer latex anu 2015 scott-morrison
